...
export default async function DashboardPage({
  ...
  return (
    <main className="min-h-screen page-bg py-8">
      ...
      {!jobs?.length ? (
        <div className="card-kumpuni border-dashed border-2 py-16 flex flex-col items-center justify-center text-center bg-transparent mt-8">
        ...
        <ul className="grid gap-5 sm:grid-cols-2">
          {jobs.map((job) => (
            <li key={job.id} className="h-full">
              <div className={`card-kumpuni h-full flex flex-col hover:-translate-y-1 transition-transform duration-300 relative overflow-hidden ${cardAccentByStatus[job.status] ?? ""}`}>
                ...
                <Link href={`/jobs/${job.id}`} className="block group flex-1 relative z-10">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    ...
                    <span className="text-[11px] font-bold text-text-secondary bg-surface-light px-2.5 py-1 rounded-md border border-subtle uppercase tracking-wider shadow-sm">
                      {CATEGORY_LABELS[job.category] ?? job.category}
                    </span>
                  </div>
                  <p className="text-lg font-extrabold text-text-primary line-clamp-2 mt-2 group-hover:text-kumpuni-blue transition-colors">
                    {job.description}
                  </p>
                  <div className="flex items-center gap-2 text-xs font-semibold text-text-tertiary mt-3 bg-white w-fit px-2.5 py-1.5 rounded-md border border-subtle shadow-sm">
                  ...
                  <div className="mt-5 flex gap-3 pt-5 border-t border-subtle relative z-10 flex-col sm:flex-row">
                    ...
                    {job.matching_mode === "fast" && (
                      <Link
                        ...
                        className="btn-primary shadow-md flex-none py-2.5"
                      >
                        Tingnan ang Matches
                      </Link>
                    )}
                ...
                {job.status === "completed" && !reviewedJobIds.includes(job.id) && (
                  <div className="mt-5 pt-5 border-t border-subtle relative z-10">
                    <Link
                      ...
                      className="btn-primary w-full shadow-md flex justify-center gap-2 !bg-success-green hover:!bg-green-700 !border-success-green"
                    >
